...
- **The `kg` Poetry group is gone** (`kgrag_priv` sweep item 50, phase 1).
  ...
  `.mcp.json`'s `pycodekg` and `dockg` servers run the tools from `PATH`
  instead of `poetry run`, which resolved them inside the venv this change
  empties. The `ftreekg` server stays on `poetry run`: it is this repo's
  own package.
...
- **`kg-rag` floor raised to `>=0.16.0`** in the `adapter` extra (was
  `>=0.14.0`), and the `ruff` floor from `>=0.4.0` to `>=0.15` inside the
  existing `<0.16` cap (`kgrag_priv` sweep item 49, tier 1). `mcp` moves to
  1.30.0 in the lock.
- **`mcp` floor raised from `>=1.0.0` to `>=1.3.0`**, keeping the `<2` cap
  (`kgrag_priv` sweep item 47). `ftreekg-mcp` passes `instructions=` to
