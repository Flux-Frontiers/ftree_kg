...
_PRE_COMMIT_HOOK = """\
#!/usr/bin/env bash
# FTreeKG pre-commit hook — runs quality checks. The index rebuild and metrics
# snapshot are opt-in and OFF by default; see "Why snapshots are off" below.
# Installed by: ftreekg install-hooks
#
#   FTREEKG_SNAPSHOT=1 git commit ...        opt in to a per-commit snapshot
#   FTREEKG_SKIP_SNAPSHOT=1 git commit ...   force snapshots off (wins)
#
# Note that FTREEKG_SKIP_SNAPSHOT no longer skips the quality checks. It used
...
# FTREEKG_SNAPSHOT=1 is set, and is skipped regardless if
# FTREEKG_SKIP_SNAPSHOT=1.
# ---------------------------------------------------------------------------
[ "${FTREEKG_SNAPSHOT:-0}" = "1" ] || exit 0
...
def install_hooks(repo: str, force: bool) -> None:
    ...
    click.echo("  Opt in with:  FTREEKG_SNAPSHOT=1 git commit ...")
    click.echo("  Force off:    FTREEKG_SKIP_SNAPSHOT=1 git commit ...")
